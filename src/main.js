...
// Gestion de la connexion avec choix du rôle
...
loginForm.addEventListener("submit", (e) => {
  ...
  const user = contacts.find(
    (c) =>
      (c.nom.toLowerCase() === nom ||
        c.prenom.toLowerCase() === nom ||
        (`${c.prenom} ${c.nom}`).toLowerCase() === nom) &&
      c.numero === numero
  );
  ...
  if (user) {
    ...
    loginError.textContent = "Nom/prénom ou numéro incorrect";
...
// Initialisation de l'état de connexion au chargement
...
function validerNouveauContact(nom, prenom, numero) {
  ...
  // Si on a dû modifier le nom, extraire les nouvelles valeurs
...
function getDernierMessage(type, id) {
  const msgs =
    type === "contact" ? messages.contacts[id] : messages.groupes[id];
  if (!msgs || msgs.length === 0)
    return { texte: "Aucun message", timestamp: new Date() };
    ...
    <div class="contact-item flex items-center p-3 hover:bg-gray-200 cursor-pointer rounded-lg transition-colors duration-200" 
      ...
      <div class="flex-1">
        <div class="flex justify-between items-center">
          <h3 class="font-semibold text-gray-800">${contact.prenom} ${
    contact.nom
  }</h3>
          <div class="flex items-center gap-2">
            <span class="text-xs text-gray-500">${formatTime(
              dernierMsg.timestamp
            )}</span>
            <div class="w-3 h-3 rounded-full ${
              estLu ? "bg-gray-400" : "bg-green-500"
            }"></div>
            ${unarchiveBtn}
          </div>
        </div>
        <p class="text-sm text-gray-600 truncate">${dernierMsg.texte}</p>
      </div>
    </div>
  `;
}
  ...
  document.querySelectorAll(".contact-checkbox").forEach((checkbox) => {
    checkbox.addEventListener("change", (e) => {
      ...
      if (e.target.checked) {
        ...
        contactsSelectionnes = contactsSelectionnes.filter(
          (id) => id !== contactId
        );
      }
...
function afficherDiscussions() {
  ...
  resultats.contacts.forEach((contact) => {
    if (!archives.contacts.includes(contact.id)) {
      // Vérifie s'il y a un brouillon
      const brouillonKey = "contact-" + contact.id;
      const hasBrouillon = brouillons[brouillonKey] && brouillons[brouillonKey].trim() !== "";
      ...
      tousLesElements.push({
        ...
        brouillon: hasBrouillon ? brouillons[brouillonKey] : null,
      });
    }
  });
  ...
  resultats.groupes.forEach((groupe) => {
    if (!archives.groupes.includes(groupe.id)) {
      // Vérifie s'il y a un brouillon
      const brouillonKey = "groupe-" + groupe.id;
      const hasBrouillon = brouillons[brouillonKey] && brouillons[brouillonKey].trim() !== "";
  ...
  tousLesElements.forEach((element) => {
    if (element.type === "contact") {
      contentSection.innerHTML += creerContactAvecBrouillon(element.data, element.brouillon);
    } else {
      contentSection.innerHTML += creerGroupeAvecBrouillon(element.data, element.brouillon);
    }
  });
...
function addUnarchiveListeners() {
  document.querySelectorAll(".unarchive-btn").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      afficherArchives();
      showNotification(
        `${type === "contact" ? "Contact" : "Groupe"} désarchivé !`
      );
    });
  });
}
...
function addContactClickListeners() {
  document.querySelectorAll(".contact-item").forEach((item) => {
    item.addEventListener("click", (e) => {
      ...
      // Sauvegarder le brouillon de la discussion en cours
      if (selectedContact) {
        brouillons["contact-" + selectedContact.id] = document.getElementById("input-message").value;
      }
      if (selectedGroupe) {
        brouillons["groupe-" + selectedGroupe.id] = document.getElementById("input-message").value;
      }
...
function updateDiscussionHeader(item, type) {
  ...
  if (type === "contact") {
    ...
    contactAvatar.className = `w-12 h-12 ${getAvatarColor(
      item.nom
...
function afficherMessages(type, id) {
  ...
  contentDiscussions.innerHTML = `
    <div class="flex flex-col gap-2 px-2 py-4">
      ${msgs
        .map(
          (msg) => `
        <div class="flex ${msg.expediteur === "moi" ? "justify-end" : "justify-start"}">
          <div class="
            max-w-[70%] 
            ${msg.expediteur === "moi" 
              ? "bg-green-500 text-white rounded-br-2xl rounded-tl-2xl rounded-bl-2xl ml-8"
              : "bg-white text-gray-800 rounded-bl-2xl rounded-tr-2xl rounded-br-2xl mr-8 border"}
            shadow p-3 relative
          ">
            ${
            ...
            <div>
              ${
                msg.audio
                  ? `<audio controls src="${msg.audio}" class="w-full my-2"></audio>`
                  : `<span class="break-words">${msg.texte}</span>`
              }
            </div>
            <div class="flex items-center gap-1 text-xs opacity-70 mt-2 justify-end">
              <span>${formatTime(msg.timestamp)}</span>
              ${
                msg.expediteur === "moi"
                  ? `<i class="fas fa-check${
                      msg.lu ? "-double text-blue-200" : " text-gray-200"
                    } text-xs ml-1"></i>`
                  : ""
              }
            </div>
  ...
  // Restaure le brouillon si présent
  const input = document.getElementById("input-message");
  const key = type + "-" + id;
  input.value = brouillons[key] || "";
}
...
function envoyerMessage(texte) {
  ...
  // Rafraîchir la liste des discussions si on est dans la vue messages
  ...
  // Efface le brouillon
  if (selectedContact) {
    delete brouillons["contact-" + selectedContact.id];
  } else if (selectedGroupe) {
    delete brouillons["groupe-" + selectedGroupe.id];
  }
}

// Fonction pour peupler la sélection de membres
...
function updateNbDestinataires() {
  document.getElementById("nb-destinataires").textContent =
    contactsSelectionnes.length;
}
...
searchInput.addEventListener("input", function () {
  ...
  if (value === "*") {
    afficherTousLesContactsAlpha();
  } else {
    // ...ta logique de recherche habituelle...
  }
});

// Fonction pour afficher tous les contacts triés alphabétiquement
function afficherTousLesContactsAlpha() {
  // contacts doit être un tableau d'objets { nom, prenom, ... }
  const contactsTries = contacts.slice().sort((a, b) => {
    const nomA = (a.nom || "").toLowerCase();
    const nomB = (b.nom || "").toLowerCase();
    return nomA.localeCompare(nomB);
  });

  // Ici, tu dois afficher la liste dans ton interface
  // Par exemple, si tu as une div avec id="content-section" :
  const contentSection = document.getElementById("content-section");
  contentSection.innerHTML = contactsTries.map(contact =>
    `<div class="p-2 border-b border-gray-200">${contact.prenom} ${contact.nom}</div>`
  ).join("");
}
...
contactForm.addEventListener("submit", (e) => {
  ...
  const newContact = {
    ...
    nom: validation.nom,
    prenom:
      validation.prenom + (validation.suffixe ? ` ${validation.suffixe}` : ""),
...
groupForm.addEventListener("submit", (e) => {
  ...
  const membresSelectionnes = Array.from(
    document.querySelectorAll(".membre-checkbox:checked")
  ...
  const newGroupe = {
    ...
    admin: membresSelectionnes[0], // Premier membre sélectionné devient admin
  };
...
document.getElementById("send-diffusion").addEventListener("click", () => {
  ...
  showNotification(
    `Diffusion envoyée à ${contactsSelectionnes.length} contacts !`
  );
});
...
document
  .getElementById("close-gestion-groupe")
...
document
  .getElementById("btn-supprimer-membre")
  .addEventListener("click", () => {
...
document.getElementById("btn-archiver-groupe").addEventListener("click", () => {
  if (
    currentGroupeGestion &&
    !archives.groupes.includes(currentGroupeGestion.id)
  ) {
...
document
  .getElementById("btn-supprimer-groupe")
  .addEventListener("click", () => {
    if (
      currentGroupeGestion &&
      ...
      // Réinitialiser la vue de discussion
      resetDiscussionView();
...
document
  .getElementById("btn-supprimer-contact")
  .addEventListener("click", () => {
    if (
      selectedContact &&
...
document
  .getElementById("btn-archiver-conversation")
  .addEventListener("click", () => {
    if (selectedContact && !archives.contacts.includes(selectedContact.id)) {
      ...
      selectedGroupe &&
      !archives.groupes.includes(selectedGroupe.id)
    ) {
...
document
  .getElementById("btn-supprimer-discussion")
  .addEventListener("click", () => {
    if (
      selectedContact &&
      confirm("Êtes-vous sûr de vouloir supprimer cette discussion ?")
    ) {
      messages.contacts[selectedContact.id] = [];
      afficherMessages("contact", selectedContact.id);
...
function resetDiscussionView() {
  ...
  document.getElementById("contact-avatar").className =
    "w-12 h-12 bg-[#d9caa3] rounded-full flex items-center justify-center";
...
document.addEventListener("click", (e) => {
  if (e.target.id === "valider-ajout-membres") {
    const selectionnes = Array.from(
      document.querySelectorAll(
        '.gestion-membre-checkbox:checked[data-action="ajouter"]'
      )
    ).map((cb) => parseInt(cb.dataset.id));

    if (selectionnes.length > 0) {
      currentGroupeGestion.membres = [
        ...currentGroupeGestion.membres,
        ...selectionnes,
      ];
  ...
  if (e.target.id === "valider-suppression-membres") {
    const selectionnes = Array.from(
      document.querySelectorAll(
        '.gestion-membre-checkbox:checked[data-action="supprimer"]'
      )
    ).map((cb) => parseInt(cb.dataset.id));

    if (selectionnes.length > 0) {
      currentGroupeGestion.membres = currentGroupeGestion.membres.filter(
        (id) => !selectionnes.includes(id)
      );
      modalGestionMembres.classList.add("hidden");
...
document
  .getElementById("close-gestion-membres")
  .addEventListener("click", () => {
    modalGestionMembres.classList.add("hidden");
    modalGestionGroupe.classList.remove("hidden");
  });
...
btnRecordVoice.addEventListener("click", async () => {
  if (!mediaRecorder || mediaRecorder.state === "inactive") {
    ...
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
      try {
        ...
        mediaRecorder.onstop = () => {
          ...
          // Ajoute le message vocal dans la conversation
          const nouveauMessage = {
            id: Date.now() + Math.random(),
            texte: "",
            audio: audioUrl,
        ...
        mediaRecorder.start();
        btnRecordVoice.classList.add("bg-red-500");
        btnRecordVoice.title = "Cliquez pour arrêter l'enregistrement";
      } catch (err) {
    ...
    mediaRecorder.stop();
    btnRecordVoice.classList.remove("bg-red-500");
    btnRecordVoice.title = "Envoyer un message vocal";
  }
});

let brouillons = {}; // { 'contact-1': 'texte', 'groupe-2': 'texte' }

function creerContactAvecBrouillon(contact, brouillon) {
  const initials = getInitials(contact.prenom, contact.nom);
  const avatarColor = getAvatarColor(contact.nom);
  const dernierMsg = getDernierMessage("contact", contact.id);
  const estLu = dernierMsg.lu;
  ...
  return `
    <div class="contact-item flex items-center p-3 hover:bg-gray-200 cursor-pointer rounded-lg transition-colors duration-200" 
      ...
      <div class="flex-1">
        ...
        <p class="text-sm truncate ${brouillon ? 'italic text-orange-600 font-semibold' : 'text-gray-600'}">
          ${brouillon ? `<i class="fas fa-pencil-alt mr-1"></i>Brouillon: ${brouillon}` : dernierMsg.texte}
        </p>
      </div>
    </div>
  `;
...
function creerGroupeAvecBrouillon(groupe, brouillon) {
  const initials = groupe.nom.substring(0, 2).toUpperCase();
  const avatarColor = getAvatarColor(groupe.nom);
  const dernierMsg = getDernierMessage("groupe", groupe.id);
  const estLu = dernierMsg.lu;
  ...
  return `
    <div class="groupe-item flex items-center p-3 hover:bg-gray-200 cursor-pointer rounded-lg transition-colors duration-200"
      ...
      <div class="flex-1">
        ...
        <p class="text-sm truncate ${brouillon ? 'italic text-orange-600 font-semibold' : 'text-gray-600'}">
          ${brouillon ? `<i class="fas fa-pencil-alt mr-1"></i>Brouillon: ${brouillon}` : `${groupe.membres.length} membres`}
        </p>
      </div>
    </div>
  `;
